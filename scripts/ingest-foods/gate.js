...
// opts.alcohol skips the macro-cal check: alcohol calories come from ethanol,
// which isn't in protein/carbs/fat, so the consistency test doesn't apply.
function gateReason(rec, opts = {}) {
  ...
  if (!opts.alcohol && kcal > 0 && Math.abs(derived - kcal) / kcal > 0.30) return "macro_cal_mismatch";
