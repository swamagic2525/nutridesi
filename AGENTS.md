...
```
Twilio WhatsApp Sandbox → Make.com → Codex Haiku API → Supabase → Twilio reply
```
...
| NLP Parser | Codex Haiku API (pay-per-use) | Cheapest model capable of Hinglish parsing |
...
Twilio requires an HTTP `200 OK` almost instantly. A single scenario calling Codex + Supabase + reply takes 4–6 seconds and causes Twilio timeouts, dropped messages, and duplicates.
...
4. Does NOT call Codex. Does NOT do any calculation.
...
3. Calls Codex Haiku API with system prompt + alias map + user message
...
## The Parser Contract (Codex Haiku Output Schema)

Every call to Codex Haiku must request this exact JSON structure. Do not deviate.
...
In a coded backend this is enforced via enum. In Make.com, it must be enforced in the schema sent to Codex. Always include this in the `quantity` property description:
...
Without this, Codex may return `0.35` or `1.25` — values Make.com's math modules cannot handle cleanly.
...
Do not pass raw JSON array to Codex. Use alias-formatted single-line entries:
...
This format uses 60–70% fewer tokens than raw JSON and puts alias strings where Codex's attention lands.
...
Apply this *before* the string reaches Codex (Make.com text processing step):
...
**Day 2:** Make.com Scenario B → picks up row → calls Codex Haiku with system prompt → structured JSON output logged to console/Supabase.
