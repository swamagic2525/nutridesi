...
function matchRows(rows, foodHints, rawMessage = "") {
  ...
  return (foodHints || []).map(hint => {
    ...
    // No name overlap (bare "it was 200 calories", or a brand rename like
    // "it was MB biozyme whey isolate" vs a row named "Protein Shake"): the
    // estimate callout invites exactly these replies, so when the batch has
    // exactly one flagged item (uncurated, else sole estimate), that item is
    // the target — not a dead-end. Gram-logged rows ("30g Milk...") are
    // excluded from the estimate tier: the user weighed those, and their
    // is_estimate flag reflects DB averages, not an assumption to correct.
    if (!best && !targetId) {
      ...
      best = uncurated.length === 1 ? uncurated[0]
